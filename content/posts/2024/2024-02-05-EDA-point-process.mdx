...
description: >-
  이번 글에서는 Point Process 데이터를 분석하는 방법에 대해 Pysal 패키지와 예제 데이터를 바탕으로 살펴보도록 하겠다. 이전에
  정리본을 영어로 업로드해두어, 간단한 정의에 대해 먼저 다시 살펴보고, Python을 활용해 intensity meausre와 같은
  point process 데이터의 주요 특징을 어떻게 분석하는지 다루어보도록 하겠다. Point Process Def...
date: "2024-02-05"
tags:
  - Spatial Statistics
  - PySAL
...
routeSegments:
  - EDA-point-process
teaser: /assets/img/Pasted%20image%2020240205155448.png
coverImage: /assets/img/Pasted%20image%2020240205155448.png
excerpt: >-
  이번 글에서는 Point Process 데이터를 분석하는 방법에 대해 Pysal 패키지와 예제 데이터를 바탕으로 살펴보도록 하겠다. 이전에
  정리본을 영어로 업로드해두어, 간단한 정의에 대해 먼저 다시 살펴보고, Python을 활용해 intensity meausre와 같은
  point process 데이터의 주요 특징을 어떻게 분석하는지 다루어보도록 하겠다. Point Process Def...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/EDA-point-process/
---

이번 글에서는 [Point Process](/statistics/Spatial-Stat-3/) 데이터를 분석하는 방법에 대해 `Pysal` 패키지와 예제 데이터를 바탕으로 살펴보도록 하겠다. 이전에 정리본을 영어로 업로드해두어, 간단한 정의에 대해 먼저 다시 살펴보고, Python을 활용해 intensity meausre와 같은 point process 데이터의 주요 특징을 어떻게 분석하는지 다루어보도록 하겠다.
...
```
![](/assets/img/Pasted%20image%2020240205143713.png)
...
![](/assets/img/Pasted%20image%2020240205155448.png)
...
```
![](/assets/img/Pasted%20image%2020240205160952.png)
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-02-05%20%EC%98%A4%ED%9B%84%202.50.15.png)
...
```
![](/assets/img/Pasted%20image%2020240205161452.png)
...
![](/assets/img/Pasted%20image%2020240205181432.png)
...
![](/assets/img/Pasted%20image%2020240205190348.png)
