...
description: >-
  Variational Inference with DP 이번 글에서는 다음 두 논문 - Jälkö, J. et al., (2017).
  Differentially Private Variational Inference for Non-conjugate Models - Wang,
  Y. et al., (2015). Privacy for Free: Posterior Sampling and Stoch...
date: "2024-05-09"
tags:
...
routeSegments:
  - DPVI
teaser: /assets/img/Pasted%20image%2020240508161802.png
coverImage: /assets/img/Pasted%20image%2020240508161802.png
excerpt: >-
  Variational Inference with DP 이번 글에서는 다음 두 논문 - Jälkö, J. et al., (2017).
  Differentially Private Variational Inference for Non-conjugate Models - Wang,
  Y. et al., (2015). Privacy for Free: Posterior Sampling and Stoch...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/DPVI/
---
...
논문 등을 바탕으로 [Variational Inference](/Variational-Inference/) 프레임워크에 DP<sup>Differential Privacy</sup>가 어떻게 적용될 수 있는지 살펴보고자 한다. 또한, 이를 Python으로 구현하여 시뮬레이션을 수행해보았다.

## Differential Privacy

DP에 대한 기본적인 정의는 [이 글](/Differential-Privacy/)을 참고하면 좋을 것 같다.
...
![](/assets/img/Pasted%20image%2020240508161802.png)
...
[Variational Bayes](/Variational-Inference/) 방법론은 ELBO로 부르는 evidence lower bound를 최대화하는 방향으로 학습하여 사후분포 $p$를 variational distribution $q$로 근사한다.
...
이러한 방법을 **doubly stochastic variational inference**라고 한다. 이를 [reparameterization trick](/VAE/)이라고도 하는데, 자동 미분(automatic differentiation, AD)를 사용하는 SGD 알고리즘에는 이러한 방법이 필요하다 (ADVI<sup>Automatic Differentiation Variational Inference</sup> 등).

### DPVI

DPVI는 Variational Inference에 DP를 적용한 방법이다. 기본적으로 [Deep Learning with Differential Privacy](/DPDL/)에서 다룬 **gradient clipping** 방법을 사용할 수 있고, 이를 Variational Inference에 적용한다. 전반적인 알고리즘은 다음과 같다.

![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-05-09%20%EC%98%A4%EC%A0%84%2010.23.33.png)
*DPVI Algorithm*
