...
description: >-
  최근 딥러닝 관련 연구동향을 살펴보던 중, 일반적으로 널리 사용되던 MLP Multi-Layer Perceptron 를 대체할 수 있는
  모델이 등장했다는 소식을 접했습니다. KAN Kolmogorov-Arnold Network 가 바로 그것인데 (Liu et al.,
  2024), 아직 저널이나 컨퍼런스에 발표되지는 않은 논문이지만 큰 화제를 모으고 있기도 합니다. 이번 글에서는 KAN의...
date: "2024-05-12"
tags:
  - Deep Learning
  - Machine Learning
...
routeSegments:
  - KAN
teaser: /assets/img/Pasted%20image%2020240512154659.png
coverImage: /assets/img/Pasted%20image%2020240512154659.png
excerpt: >-
  최근 딥러닝 관련 연구동향을 살펴보던 중, 일반적으로 널리 사용되던 MLP Multi-Layer Perceptron 를 대체할 수 있는
  모델이 등장했다는 소식을 접했습니다. KAN Kolmogorov-Arnold Network 가 바로 그것인데 (Liu et al.,
  2024), 아직 저널이나 컨퍼런스에 발표되지는 않은 논문이지만 큰 화제를 모으고 있기도 합니다. 이번 글에서는 KAN의...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/KAN/
---
...
MLP는 현재 딥러닝이라고 불리는 학습 이론들의 근간이 되는 모델입니다. 가중치 행렬 $W$를 사용한 **선형**결합과 **비선형** 활성함수 (ex. ReLU)를 이용해 임의의 비선형 함수를 근사합니다. 이는 [Universal approximation theorem](/Universal-Approximation-Theorem/)으로 설명되는데, 이는 임의의 비선형함수를 주어진 오차 내에서 몇개의 레이어와 몇개의 깊이(depth)를 갖는 완전연결 네트워크(fully-connected network, FCN)으로 근사할 수 있음을 의미합니다. 

반면, KAN의 경우 **Kolmogorov-Arnold representation theorem**을 근간으로 모델을 전개합니다. 무언가 거창한 정리와 모델로 보일 수는 있지만, 저자는 KAN이 *nothing more than combinations of splines and MLP*, 즉 [spline](/machine%20learning/Splines/) 모델과 MLP를 결합시킨 것에 불과하다고 합니다. 
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-05-12%20%EC%98%A4%EC%A0%84%2011.12.12.png)
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-05-12%20%EC%98%A4%EC%A0%84%2011.06.22.png)
...
또한, 각각의 함수 $\phi_{q,p},\Phi$ 들은 학습이 가능해야 하므로 [spline](/machine%20learning/Splines/)을 이용하여, 각 함수들의 계수(coefficient)를 학습합니다. KAN 에서는 **B-spline**을 이용합니다. (ex. Spline 포스트의 natural cubic spline 참고)
...
KAT의 핵심은 **approximation bound가 입력 차원 N에 의존하지 않는다**는 것입니다. 즉, spline의 가정 형태만 조절하면, 임의의 함수를 잘 근사할 수 있게 됩니다. 논문에서는 $k=3$, 즉 [cubic spline](/machine%20learning/Splines/)을 사용하는 것이 적절하다고 제안한 반면, grid size $G$의 경우는 너무 fine하게 설정할 경우 test loss가 발산하는 overfitting 문제가 존재하는 것을 보였습니다. (아래) 만일 training data가 $n$개 있고, 전체 parameter 수가 $mG$개로 계산된다면, $G=\dfrac{n}{m}$ 을 넘지 않도록 설정하는 것이 적합하다고 합니다 (interpolation threshold).

![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-05-12%20%EC%98%A4%ED%9B%84%201.34.36.png)
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-05-12%20%EC%98%A4%ED%9B%84%201.46.54.png)
#### Sparsification

MLP에서는 [regularization](/deep%20learning/Regularization_on_DL/) 기법을 이용해 가중치에 sparsity를 부여하였습니다. 이와 유사하게 KAN에서는 다음과 같이 레이어의 L1 노음을 정의합니다.
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-05-12%20%EC%98%A4%ED%9B%84%202.03.53.png)
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-05-12%20%EC%98%A4%ED%9B%84%202.18.38.png)
*Some symbolic tasks*
...
![](/assets/img/Pasted%20image%2020240512154659.png)
*Sample dataset*
...
![](/assets/img/Pasted%20image%2020240512154843.png)
...
![](/assets/img/Pasted%20image%2020240512155037.png)
...
model.plot()
```
![](/assets/img/Pasted%20image%2020240512155111.png)
...
model2.plot()
```
![](/assets/img/Pasted%20image%2020240512155147.png)
...
![](/assets/img/Pasted%20image%2020240512155806.png)
...
model.plot()
```
![](/assets/img/Pasted%20image%2020240512160046.png)
...
![](/assets/img/Pasted%20image%2020240512160203.png)
*Accuracy plot*

또한, 가지치기를 하면 다음과 같이 $[4,4,3]$-KAN으로 줄어든 것을 확인할 수 있습니다.

![](/assets/img/Pasted%20image%2020240512160241.png)
