...
description: >-
  Cox Process Point Pattern 데이터를 모델링하는 일반적인 방법은 intensity function을 추정하는 것이다.
  이때, intensity function이 $\lambda(x)=2\exp(-x)$ 와 같은 함수 형태로 주어질 수도 있지만, 이러한 함수를
  직접 추정하는 것 대신 함수의 형태에 대해서도 추가적인 랜덤성을 가정하는 것이 가능하다. 이러한 방법을 Cox...
date: "2024-05-07"
tags:
  - Spatial Statistics
  - Point Process
...
coverImage: /assets/img/coximg1.png
excerpt: >-
  Cox Process Point Pattern 데이터를 모델링하는 일반적인 방법은 intensity function을 추정하는 것이다.
  이때, intensity function이 $\lambda(x)=2\exp(-x)$ 와 같은 함수 형태로 주어질 수도 있지만, 이러한 함수를
  직접 추정하는 것 대신 함수의 형태에 대해서도 추가적인 랜덤성을 가정하는 것이 가능하다. 이러한 방법을 Cox...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/Coxprocess/
---

# Cox Process

[Point Pattern](/EDA-point-process/) 데이터를 모델링하는 일반적인 방법은 intensity function을 추정하는 것이다. 이때, intensity function이 $\lambda(x)=2\exp(-x)$ 와 같은 함수 형태로 주어질 수도 있지만, 이러한 함수를 직접 추정하는 것 대신 함수의 형태에 대해서도 추가적인 랜덤성을 가정하는 것이 가능하다. 이러한 방법을 **Cox Process**라고 한다. 이번 글에서는 Cox process에 대한 정의와 **log Gaussian Cox process**를 **Hamiltonian Monte Carlo**로 추정하는 방법에 대해서 다루고자 한다.
...
앞서 주어진 log posterior를 직접 최대화하는 것은 어렵기 때문에, 베이지안 추론에서는 사후분포를 근사하는 방법을 사용한다. [VB](/Variational-Inference/), [MCMC](/statistics/MCMC/) 등 다양한 방법이 존재하지만 여기서는 MCMC 중 **Hamiltonian Monte Carlo**를 사용하여 사후분포를 추정하는 방법을 다루고자 한다.

HMC는 [Metropolis-Hastings](/statistics/MCMC/) 알고리즘을 개선한 방법으로, 파라미터들이 높은 상관성을 가질 때 특히 효과적이다. HMC는 (물리학에서 비롯된) *Hamiltonian dynamics*를 사용하여 파라미터 공간을 탐색하는데, 이때 Hamiltonian은 다음과 같이 주어진다.
...
Python에서는 `pymc` 패키지를 사용하여 HMC를 구현할 수 있다. 패키지의 자세한 사용법은 [이전 글](/pymc/)을 참고하면 된다.
...
![](/assets/img/Pasted%20image%2020240507190549.png)
*Trace Plot*
...
![](/assets/img/Pasted%20image%2020240507190933.png)
