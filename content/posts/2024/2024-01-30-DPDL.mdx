...
description: >-
  DL with DP 이번 포스트에서는 Deep Learning with Differential Privacy (Abadi et al.,
  2016) 논문을 리뷰하고자 합니다. 이 논문은 neural network 기반의 딥러닝 모델을 Differential Privacy를
  만족하도록 학습하는 방법을 제시합니다. 이 과정에서 Stochastic gradient descent (SGD) 자체...
date: "2024-01-30"
tags:
  - Deep Learning
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/DPDL/
---

# DL with DP

이번 포스트에서는 *Deep Learning with Differential Privacy (Abadi et al., 2016)* 논문을 리뷰하고자 합니다. 이 논문은 neural network 기반의 딥러닝 모델을 Differential Privacy를 만족하도록 학습하는 방법을 제시합니다. 이 과정에서 [Stochastic gradient descent](/deep%20learning/Optimization/) (SGD) 자체가 DP를 만족하도록 하는 방법인 **DP-SGD**를 제시하고, 이는 여러 후속 연구에서도 많이 활용되고 있습니다.

## Definitions

### Differential Privacy

Randomized mechanism<sup>algorithm</sup> $\mathcal{M}:\mathcal{D}\to \mathbb{R}$ 이 데이터셋의 정의역 $\mathcal{D}$ 와 output range $\mathcal{R}$ 를 갖는다고 하자. 이때 메커니즘 $\mathcal{M}$ 이 $(\epsilon,\delta)$-DP인 것은 임의의 두 [이웃](/Differential-Privacy/)하는 데이터셋 $D,D'\in\mathcal{D}$ 에 대해
...
> DP에 대한 자세한 내용은 [Differential Privacy](/Differential-Privacy/), [Renyi Differential Privacy](/RDP/) 등의 포스트를 참고하시기 바랍니다.
...
일반적으로 최적화 문제 $\min_{\theta}\mathcal{L}(\theta)$ 를 풀기 위해서 Newton's method나 L-BFGS와 같은 최적화 알고리즘을 사용한다. 그러나 Hessian matrix를 계산하는 것은 일반적인 딥러닝 세팅(large $N$)에서는 매우 비효율적이다. 따라서, [확률적 경사하강법](/deep%20learning/Optimization/) 기반의 알고리즘을 사용한다.
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202024-07-02%20%EC%98%A4%EC%A0%84%209.19.01.png)
