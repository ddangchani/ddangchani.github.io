...
description: >-
  Differential Privacy Setting $D=(d{1},\ldots,d{n})\in \mathcal{D}$을 입력
  데이터베이스라고 하자. 또한, $M:\mathcal{D} \to \mathbb{R}^{d}$를 non-private 알고리즘이라고 하고,
  $M(D)$를 그에 대응하는 출력이라고 하자. 그러면, 출력 공간 (e.g. $\mathbb{R}^{d}$)을 고려하여, 랜...
date: "2024-01-31"
tags:
  - Paper Review
...
routeSegments:
  - DP-for-Functions
teaser: /assets/img/Pasted%20image%2020240131190247.png
coverImage: /assets/img/Pasted%20image%2020240131190247.png
excerpt: >-
  Differential Privacy Setting $D=(d{1},\ldots,d{n})\in \mathcal{D}$을 입력
  데이터베이스라고 하자. 또한, $M:\mathcal{D} \to \mathbb{R}^{d}$를 non-private 알고리즘이라고 하고,
  $M(D)$를 그에 대응하는 출력이라고 하자. 그러면, 출력 공간 (e.g. $\mathbb{R}^{d}$)을 고려하여, 랜...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/DP-for-Functions/
---
...
RKHS에 대한 내용은 다음 [포스트](/machine%20learning/kernel2/)를 참고하자.
...
![](/assets/img/Pasted%20image%2020240131190247.png)
