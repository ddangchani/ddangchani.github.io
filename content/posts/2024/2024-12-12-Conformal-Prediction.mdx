...
description: >-
  Conformal Prediction Introduction 일반적으로 통계학에서는 선형모형과 같은 모델을 설정하고, 이를 바탕으로
  설명변수와 예측변수 간의 관계를 분석한다. 나아가, 학습한 모델을 바탕으로 새로운 데이터 $x^\ast$가 주어졌을 때, 이에 대한 예측값
  $\hat{y}^\ast$를 계산한다. 예를 들어, 데이터 $X=(x1, x2, \cdots, x{n}), Y=(y{...
date: "2024-12-12"
tags:
  - Machine Learning
...
routeSegments:
  - Conformal-Prediction
teaser: /assets/img/Pasted%20image%2020241213172108.png
coverImage: /assets/img/Pasted%20image%2020241213172108.png
excerpt: >-
  Conformal Prediction Introduction 일반적으로 통계학에서는 선형모형과 같은 모델을 설정하고, 이를 바탕으로
  설명변수와 예측변수 간의 관계를 분석한다. 나아가, 학습한 모델을 바탕으로 새로운 데이터 $x^\ast$가 주어졌을 때, 이에 대한 예측값
  $\hat{y}^\ast$를 계산한다. 예를 들어, 데이터 $X=(x1, x2, \cdots, x{n}), Y=(y{...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/Conformal-Prediction/
---
...
![](/assets/img/Pasted%20image%2020241212135952.png)
...
![](/assets/img/Pasted%20image%2020241213172108.png)
