...
description: >-
  위상공간의 분리 Separation 위상공간 X의 부분집합 $A,B \subset X$ 가 서로소라고 하자. 만일 $A,B$ 각각의 서로소인
  근방이 존재한다면, 이를 근방에 의해 분리된다고 표현한다. 이 장에서는 네 가지의 주요 분리 성질을 바탕으로 위상공간을 분류하는 것을
  다룬다. 티호노프 Tychonoff 분리 성질 위상공간 $X$의 두 점 $u,v \in X$에 대해 $v$를 포함...
date: "2021-12-28"
tags:
  - Mathematics
  - Real Analysis
...
routeSegments:
  - mathematics
  - realanalysis_Topological_spaces(2)
teaser: null
coverImage: null
excerpt: >-
  위상공간의 분리 Separation 위상공간 X의 부분집합 $A,B \subset X$ 가 서로소라고 하자. 만일 $A,B$ 각각의 서로소인
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/mathematics/realanalysis_Topological_spaces(2)/
---
