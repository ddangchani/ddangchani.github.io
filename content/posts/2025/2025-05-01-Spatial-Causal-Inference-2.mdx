...
description: >-
  이전 글에서는 처치변수와 결과변수가 모두 점 패턴인 경우에 대한 인과추론 방법론을 소개했습니다. 이번 글에서는 처치변수는 점 패턴으로
  주어지지만 결과변수가 격자형으로 주어지는 areal data에 대한 인과추론 방법론을 소개합니다. 사실 많은 사회적 변수들이 areal
  data로 수집되는 경우가 많기 때문에 (ex. 행정구역별 통계, 격자별 인구 등) 이 논문에서 제안하는 방법론은 매우...
date: "2025-05-01"
tags:
  - Causal Inference
  - Spatial Statistics
...
categories: []
routeSegments:
  - Spatial-Causal-Inference-2
teaser: >-
  /assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202025-05-01%20%EC%98%A4%ED%9B%84%2010.10.28.png
coverImage: >-
  /assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202025-05-01%20%EC%98%A4%ED%9B%84%2010.10.28.png
excerpt: >-
  이전 글에서는 처치변수와 결과변수가 모두 점 패턴인 경우에 대한 인과추론 방법론을 소개했습니다. 이번 글에서는 처치변수는 점 패턴으로
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202025-04-30%20%EC%98%A4%ED%9B%84%208.56.12.png)
...
또한, 논문에서는 인과관계를 모델링하기 위해 다음과 같이 DAG<sup>Directed Acyclic Graph</sup>을 정의합니다 ([참고 : DAG](/causal%20inference/Causal_Inference_(5)/)).
...
이때, 위 인과효과는 노이즈 변수 $\epsilon$와 숨겨진 변수 $H$에 대한 기대값을 취한 평균 효과입니다. 하지만 처치 분포 $P_x$를 직접적으로 알 수 없기 때문에, [Fubini의 정리](/mathematics/Product_Integral/)를 이용하여 다음과 같은 관계를 유도하고 이를 구할 수 있습니다.
...
![](/assets/img/%EC%8A%A4%ED%81%AC%EB%A6%B0%EC%83%B7%202025-05-01%20%EC%98%A4%ED%9B%84%2010.31.10.png)
*논문에서 보인 검정 결과*
