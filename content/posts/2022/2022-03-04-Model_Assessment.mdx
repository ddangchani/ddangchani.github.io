...
description: >-
  How to make an assessment of a model? 우리가 어떤 머신러닝 모델을 만들었을 때, 모델의 성능은 어떻게 측정할
  수 있을까🤔? 간단히 생각해보면, 서로 다른 데이터셋들에 대해 모델의 정확도를 측정하고, 이들을 종합해서 지표화하면 될 것이다(이때
  데이터셋들은 확률적으로 독립이어야 할 것이다). 모델의 성능을 측정하는 것은 매우 중요한 문제이다. 실제로 모델을...
date: "2022-03-04"
tags:
  - Machine Learning
  - Model Assessment
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/machine%20learning/Model_Assessment/
---
