...
title: Random Forest
description: >-
  Random Forest Random Forest는 Bagging 배깅 방식을 이용한 Tree algorithm의 일종이다. 즉, 서로
  상관관계가 없는 (de-correlated, randomized) tree들을 매우 많이 생성하여, 이들의 평균값을 바탕으로 분류 혹은
  회귀를 진행하는 알고리즘이다. Tree model이 Bagging algorithm을 실행하는데 가장 최적인 이유는...
date: "2022-04-14"
tags:
  - Machine Learning
  - Tree
...
coverImage: /assets/img/RandomForest.assets/RandomForest_0.png
excerpt: >-
  Random Forest Random Forest는 Bagging 배깅 방식을 이용한 Tree algorithm의 일종이다. 즉, 서로
  상관관계가 없는 (de-correlated, randomized) tree들을 매우 많이 생성하여, 이들의 평균값을 바탕으로 분류 혹은
  회귀를 진행하는 알고리즘이다. Tree model이 Bagging algorithm을 실행하는데 가장 최적인 이유는...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/machine%20learning/RandomForest/
---
