...
description: >-
  LDA의 다른 관점 이전 포스트에서 살펴본 것 처럼, LDA는 데이터셋의 정규성을 가정하여 분류하는 방법이다. 이를 제한된 가우시안 분류기
  restricted gaussian classifier 라는 관점으로 본다면, 이번에 다룰 내용은 LDA가 데이터를 저차원으로 투영시켜
  본다는 정사영 projection 의 관점이다. 클래스가 $K$개인 데이터셋이 있고, 각각의 데이터는 $p$개의...
date: "2022-02-08"
tags:
  - Linear Model
  - LDA
...
coverImage: /assets/img/lda2_0.png
excerpt: >-
  LDA의 다른 관점 이전 포스트에서 살펴본 것 처럼, LDA는 데이터셋의 정규성을 가정하여 분류하는 방법이다. 이를 제한된 가우시안 분류기
  restricted gaussian classifier 라는 관점으로 본다면, 이번에 다룰 내용은 LDA가 데이터를 저차원으로 투영시켜
  본다는 정사영 projection 의 관점이다. 클래스가 $K$개인 데이터셋이 있고, 각각의 데이터는 $p$개의...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/linear%20model/lda2/
---
