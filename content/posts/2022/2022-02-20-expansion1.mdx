...
description: >-
  Basis Expansion 기저 확장이란 데이터셋 $\mathbf{X}$의 각 벡터 $X\in\mathbf{X}$에 새로운 변수를
  추가하거나, 기존 변수를 대체하는 방법으로 새로운 모형을 구성하는 것이다. 총 M개의 변환이 존재한다고 하고, 이때 $m$번째($m=
  1,\ldots,M$) 변환을 $$ hm(X) : \mathbb{R^p\to R} $$ 로 표기하자(기존 데이터는 p개의...
date: "2022-02-20"
tags:
  - Linear Model
  - Machine Learning
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/linear%20model/expansion1/
---
