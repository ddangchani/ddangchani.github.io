...
description: >-
  Partial Regression Linear Regression model에서 예측변수가 여러개일 때, 즉 multiple linear
  regression인 경우 각각의 변수 고유의 영향력을 파악하는 방법으로 partial regression이 있다(Partial Least
  Square algorithm과 명백히 다르다!). 이에 대해 간단히 다루어보도록 하자. 우선 다음과 같은 회귀...
date: "2022-04-09"
tags:
  - Linear Model
  - Regression
...
routeSegments:
  - linear model
  - Partial_Regression
teaser: /assets/img/Partial%20Regression.assets/Partial_Regression_0.png
coverImage: /assets/img/Partial%20Regression.assets/Partial_Regression_0.png
excerpt: >-
  Partial Regression Linear Regression model에서 예측변수가 여러개일 때, 즉 multiple linear
  regression인 경우 각각의 변수 고유의 영향력을 파악하는 방법으로 partial regression이 있다(Partial Least
  Square algorithm과 명백히 다르다!). 이에 대해 간단히 다루어보도록 하자. 우선 다음과 같은 회귀...
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/linear%20model/Partial_Regression/
---
...
![스크린샷 2022-04-09 오후 7.36.48](/assets/img/Partial%20Regression.assets/Partial_Regression_0.png){: .align-center}
