...
description: >-
  Bayesian Causal Inference Setting 각 샘플 단위(개체)에 대해 다음과 같이 네 가지 변수가 존재한다. $$
  Y{i}(0), Y{i}(1), Z{i}\in\lbrace 0,1\rbrace ,X{i} $$ 이때 결과변수 $Y{i}$에 대해서는 하나만
  관측된다. 즉, $$ Y{i}^{mis}=Y{i}(1-Z{i}) $$ 의 관계가 성립한다. 그렇다면, 주어진 처치변...
date: "2023-09-10"
tags:
  - Causal Inference
  - Bayesian
...
featured: false
draft: false
canonicalUrl: https://ddangchani.github.io/Bayesian-Causal-Inference/
---
