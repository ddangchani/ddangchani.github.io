...
const tagFilterButtonBaseClassName =
  "relative inline-flex w-full items-center gap-[0.6rem] rounded-full px-[0.8rem] py-[0.4rem] pl-[1.28rem] font-medium shadow-[inset_0_1px_0_color-mix(in_srgb,white_80%,transparent),0_10px_20px_color-mix(in_srgb,var(--ink)_4%,transparent)] transition duration-200 ease-out before:absolute before:left-[0.76rem] before:top-1/2 before:h-[0.42rem] before:w-[0.42rem] before:-translate-y-1/2 before:rounded-full sm:w-auto sm:justify-center max-[480px]:justify-between";
...
const tagFilterCountBaseClassName =
  "min-w-[1.55rem] rounded-full border px-[0.45rem] py-[0.15rem] text-[0.76rem] shadow-[inset_0_1px_0_color-mix(in_srgb,white_72%,transparent)]";
...
export function TagFilterButton({ name, count, isActive, onClick }: TagFilterButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
    >
      <span className="ml-[0.5rem] text-[0.92rem]">{name}</span>
      <span
        className={clsx(
