...
여기서 $JSD(p,q)$ 는 **Jensen-Shannon divergence**를 의미한다. 즉, 위 변환으로부터 BCE 손실함수의 목적함수를 최대화하는 문제가 Jensen-Shannon divergence를 최대화하는 문제로 해석될 수 있음을 알 수 있다. 따라서, *암시적 모델*에서 분포에 대한 직접적인 계산 없이도 샘플을 이용한 이진분류문제를 해결하면 분포가 학습이 가능해진다. 즉, 앞서 살펴본 조건 1-3을 모두 만족한다.
