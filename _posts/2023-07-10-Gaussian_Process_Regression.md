...
Gaussian process는 [정의](https://ddangchani.github.io/Gaussian-Process-1-fhaqp3yl)에 의해 임의의 input point들의 finite subset $X_{n}=\{x_{1},\ldots,x_{n}\}$ 에 대한 evaluation $f(x_{1}),\ldots,f(x_{n})$ 이 Multivariate gaussian distribution을 따르므로, 다음과 같이 주어진 Gaussian process로 부터 random Gaussian vector를 생성할 수 있다.
...
에 대해 $f$를 추정하고 이를 바탕으로 새로운 test data $X_{\star}$에 대한 output vector $\mathbf{f}_{\star}$를 예측해야 하는 문제를 생각해보자. 우선 함수 $f$는 Gaussian process $GP(0,K)$ 를 따른다고 설정하면 관측치 $X$에 대한 evaluation은 random gaussian vector
...
![](/assets/img/Pasted image 20230710174145.png){: .align-center}
