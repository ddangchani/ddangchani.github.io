...
3. 이제 이를 임의의 가측집합 $E$로 확장하자. $L^p(E)$ 에서의 유계선형범함수 $T$ 를 잡고, $L^p(\R)$ 에서 $\hat{T}(f)=T(f\vert _E)$ 를 만족하는 선형범함수 $\hat{T}$ 를 잡자. 그러면 증명의 2번 내용들로부터 $\hat{g}\in L^q(\R)$ 이 존재하여 $\hat{T}(f) = \int_\R \hat{g}\cdot f$ 를 만족한다, 함수 $g$ 를 $\hat{g}\vert _E$ 로 정의하면 $T = \mathcal{R}_g$ 가 성립한다.
