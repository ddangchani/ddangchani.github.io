...
DP 메커니즘으로 deterministic한 실함수 $f:\mathcal{D}\to \mathbb{R}$ 를 근사하는 일반적인 메커니즘은 **Gaussian mechanism**이다. 이는 다음과 같이 함수에 Gaussian noise를 추가하는 것이다.
...
> 자세한 내용은 [Differential Privacy]({% post_url 2024-01-02-Differential-Privacy %}) 포스트를 참고하시기 바랍니다.
...
Algorithm 1가 DP를 보장한다는 것을 확인하기 위해서 그래디언트의 유계성이 필요한데, 이를 위해 논문에서는 **noise clipping**을 사용한다. 이는 그래디언트의 L2 norm이 $\sigma$를 넘지 않도록 하는 것인데, 다음과 같이 정의된다.
...
DP-SGD를 활용한 python 구현 예시는 다음 게시글을 참고 : [Differentially Private Variational Inference]({% post_url 2024-05-09-DPVI %})
