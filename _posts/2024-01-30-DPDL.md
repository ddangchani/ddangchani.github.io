...
이번 포스트에서는 *Deep Learning with Differential Privacy (Abadi et al., 2016)* 논문을 리뷰하고자 합니다. 이 논문은 neural network 기반의 딥러닝 모델을 Differential Privacy를 만족하도록 학습하는 방법을 제시합니다. 이 과정에서 stochastic gradient descent(SGD) 자체가 DP를 만족하도록 하는 방법을 제시하고, 이는 여러 후속 연구에서도 많이 활용되고 있습니다.
...
> 자세한 내용은 [Differential Privacy]({% post_url 2024-01-02-Differential-Privacy %}), [Renyi Differential Privacy]({% post_url 2024-05-13-RDP %}) 등의 포스트를 참고하시기 바랍니다.
...
일반적으로 Newton's method나 L-BFGS와 같은 최적화 알고리즘을 사용하여 $\theta$를 찾는다. 그러나 Hessian matrix를 계산하는 것은 일반적인 딥러닝 세팅(large $N$)에서는 매우 비효율적이다. 따라서, SGD<sup>Stochastic Gradient Descent</sup>나 Adam과 같은 [확률적 경사하강법]({% post_url 2022-03-09-Optimization %}) 기반의 알고리즘을 사용한다.

이는 전체 데이터로부터 랜덤한 부분집합인 배치<sup>batch</sup> $B$를 만들고 다음과 같이 그래디언트를 계산한다.
...
그러나 strong composition theorem의 bound는 tight하지 않다. 이 논문에서는 **moments accountant**라는 더 강력한 계산 방법을 제시한다.  Moments accountant 정리를 이용하면, 적절한 노이즈와 clipping threshold를 선택하여 (이후 설명 참고) 위 알고리즘이 $(O(q \epsilon\sqrt{T}),\delta)$-DP 를 만족함을 보일 수 있다. 우선, 정리는 다음과 같이 주어진다.
...
이제 위 정리 1을 설명하기 위해 다음과 같은 정의들과 보조정리들을 이용한다.
#### Privacy Loss
...
만일 위 randomized mechanism이 $(\epsilon,\delta)$-DP를 만족한다면, 이는 $\mathcal{M}$의 privacy loss random variable가 특정한 **tail bound** (꼬리확률이 유계)를 갖는다는 것과 동치이다.
...
Privacy loss random variable $c$의 log moment generating function<sup>LMGF</sup>은 다음과 같이 정의된다. (통계학에서는 cumulant generating function<sup>CGF</sup> 라고도 부름)
...
앞서 privacy loss random variable의 tail bound가 DP와 동치라고 하였기 때문에, CGF의 bound를 다루는 것이 유용하다. 따라서 다음과 같이 CGF의 upper bound를 정의하기로 하자.
...
### Moment bound of each adaptive mechanism

그렇다면 각각의 adaptive mechanism $\mathcal{M}_{i}$들에 대한 moment bound를 구해야 하는데, 이는 random sampling과 [Gaussian mechanism]({% post_url 2024-05-13-RDP %})을 이용하면 된다. 구체적인 방법은 다음과 같다.

우선, $N(0,\sigma^{2})$의 확률밀도함수를 $\mu_{0}$ 라고 두고, $N(1, \sigma^{2})$의 밀도함수를 $\mu_{1}$이라고 두자. 또한, 두 밀도함수의 혼합분포 $\mu=(1-q)\mu_{0}+ q \mu_{1}$ 을 고려하자. 그러면 $\alpha(\lambda)$는 다음과 같이 계산할 수 있다.

$$
\begin{align}\\
\alpha(\lambda) &= \log \max(E_{1},E_{2})\\
E_{1}&=  \mathrm{E}_{z\sim \mu_{0}}\left[\left(\frac{\mu_{0}(z)}{\mu(z)}\right)^{\lambda}\right],\\
E_{2}&= \mathrm{E}_{z\sim \mu}\left[\left(\frac{\mu(z)}{\mu_{0}(z)}\right)^{\lambda}\right]
\end{align}
$$

이를 실제로 계산하는 과정은 수치 적분을 이용한다.
