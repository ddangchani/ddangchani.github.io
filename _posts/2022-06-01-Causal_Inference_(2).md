...
$$
P_{N_B|T=B=1}= P_{N_T|T=B=1} = \delta_1
$$


이다. 이로부터 다음과 같은 수정된 SCM
$$
\mathfrak C\;|\;B=T=1 :\begin{cases} T:=1\\ B:= T+ (1-T)\cdot 0 = T\end{cases}
$$


을 얻을 수 있는데, 이때 counterfactual에 해당하는 진술은 $T=0$이므로, 이에 대한 intervention $do(T:=0)$에 대한 SCM은
$$
\mathfrak C\;|\; B=T=1 : do(T:=0) :\begin{cases} T:=0\\ B:= T = 0\end{cases}
$$
