...
use_math: true
---
- [Weak Sequential Compactness<sup> 약한 점열 컴팩트성</sup>](#weak-sequential-compactnesssup-약한-점열-컴팩트성sup)
  - [헬리<sup>Helley</sup>의 정리](#헬리suphelleysup의-정리)
...
> 증명. 노음선형공간 $X$가 separable 하므로, 조밀한 가산부분집합 {$f_j:j\in\N$} 을 생각하자. 이때 선형범함수열 $T_n$이 유계이므로, 실수열 {$T_n(f_1) : n \in \N$} 을 생각하면 이는 유계실수열이다. 따라서 볼차노-바이어슈트라스 정리에 의해 수렴하는 부분수열이 존재하므로,   
> $$\lim_n T_{s(1,n)}(f_1) = a_1
> $$ 을 만족하는 인덱스 수열(자연수열) {$s(1,n)$}과 수렴값 $a_1$ 을 잡을 수 있다. 또한, 이렇게 생성된 부분수열 <$T_{s(1,n)}$> 역시 유계수열이므로, 수렴하는 수열 $\lim_n T_{s(2,n)}(f_2) = a_2$ 을 같은 방법으로 잡을 수 있다. 귀납적으로 이러한 인덱스 수열 $$\{s(j,n)\}_{j,n\in \N}$$ 을 구성해나가자.   
...
#### 정리 14의 증명
먼저 정리의 조건에서와 같이 $f_n$ 을 $L^p(E)$ 공간에서의 유계함수열이라고 생각하자. 또한, $X = L^q(E)$ 로 두고, $X$에서의 선형범함수열 $T_n:X\to \R$ 을   
$$
