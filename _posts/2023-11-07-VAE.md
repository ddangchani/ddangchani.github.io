...
&\geq \int \bigg[\log p_{\phi}(x\vert z)-\log q_\theta(z\vert x)+\log q_{\theta}(z\vert x)+\log p(z)\bigg]dz\\
&= \mathrm{E}_{q}\bigg[\log p_{\phi}(x\vert z)\bigg] - KL(q_{\theta}(z\vert x)\Vert p(z))
\end{align}
