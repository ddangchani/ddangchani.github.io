...
> (N2) $\Vert\alpha f\Vert = |\alpha|\cdot \Vert f \Vert$
...
1. $f\cdot g$ 는 적분가능하고   
   $$\int_E|f \cdot g| \leq \Vert f\Vert_p\cdot\Vert g\Vert_q$$ 이다.

2. $f \neq 0$ 일 떄, $$f^*=\Vert f\Vert_p^{1-p}\cdot sgn(f) \cdot |f|^{q-1}$$ 
...
> $$
\int_E|f\cdot g| \leq \Vert f\Vert_1\cdot\Vert g\Vert_\infty
...
$$
|f\cdot g| \leq |f|^p/p + |g|^q/q 
$$ 가 성립한다. 따라서 양변을 적분하면,
$$\begin{aligned}
\int_E|f\cdot g| &\leq \frac{1}{p}\int_E|f|^p + \frac{1}{q}\int_E|g|^q \\ &= \frac{1}{p}
