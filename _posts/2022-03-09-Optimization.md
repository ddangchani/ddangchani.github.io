...
우선 데이터셋 $(x,y)$가 확률분포 $P(x,y)$로부터 비롯되었다고 가정하고, 손실함수가 규제항 $R$을 포함한 형태로 식 (2)와 같이 주어졌다고 생각할 수 있다. 이때 기댓값은 표본평균으로 근사할 수 있으므로, 데이터 샘플 $\lbrace (x_i,y_i):i=1,\ldots,N\rbrace $ 에 대해 다음 식 (3)처럼 근사할 수 있다.
