...
**THM7** $L^p(E)$에서 $f_n$ 이 $f$로 E의 거의 모든 점에서 점별수렴하면,   
 함수열 $f_n$의 $f$로의 수렴과 $\lim_n\int_E|f_n|^p = \int_E|f|^p$ 임은 동치이다. 
...
> 각 자연수 n에 대해 함수족 $\mathcal{F_n}$을 $[-n,n]$ 밖에서는 소멸(vanish)하고, $[-n,n]$ 에서는 $S'[a,b]$ 에 속하는 함수들의 모임으로 정의하자. 또한, $\mathcal{F} = \cup_{n\in \N}\mathcal{F_n}$ 으로 두면 $\mathcal{F}$는 $L^p(\R)$에 속한 함수들의 가산집합이다.   
> 단조수렴정리에 의해 임의의 $f\in L^p(\R)$ 에 대해  
> $$
\lim_{n \to \infty} \int_{[-n,n]}|f|^p = \int_\R|f|^p
> $$
> 이므로 $\mathcal{F}$ 는 가산집합이며 $L^p(\R)$에서 조밀하다. 
...
> pf. 만약 separable 하다고 가정하자. 그러면 가산함수족 $f_{n \in \N}$ 이 존재하여 $L^\infty[a,b]$에서 조밀하다.  
