...
$$
\text{Err}_{in}=\frac{1}{N}\sum_{i=1}^NE_{Y^0}[L(Y_i^0,\hat f(x_i))|\mathcal T]
$$
...
> $$
> \begin{aligned}
> \text{AIC}(S)&=\log(2\pi\hat\sigma^2_S)+\frac{\Vert Y-X\hat\beta_S\Vert^2}{n\hat\sigma_S^2}+\frac{2(|S|+1)}{n}\\
> &=\log\text{SSE}(S) + {2|S|\over n}
> \end{aligned}
> $$
