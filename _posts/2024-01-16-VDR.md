---
title: A Neural Approach to Spatiotemporal data release with DP
tags: 
...
1. Range count queries 
	...
	- Metric : relative error metric $\text{RE}(y,u) = \dfrac{|y-u|}{\max\{u,\psi\}}$ where $\psi$ is smoothing factor to avoid division by zero.
...
\text{sMAPE} = \frac{1}{h}\sum_{t=1}^{h}\dfrac{|F_{t}-A_{t}|}{(A_{t}+F_{t})/2}
...
- $L_{C}(\hat T) = \sum_{i=1}^{|\bar T|}\Vert \bar T_{i}-\hat T_{i}\Vert^{2}$ : reconstruction loss
...
where $N=|D|$ is the total number of data points and $n=|D_{s}|$ is the total number of sampled data. Thus, the MSE is given by
