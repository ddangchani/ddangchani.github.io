...
행렬에서는 고유값과 고유벡터가 행렬에 크기만큼 존재하는 것과 달리, 고유함수의 경우 일반적으로 **무한개**의 고유함수가 존재합니다. (함수가 무한차원의 벡터로 표현되기 때문)
...
여기서 $a=1/4\sigma^2$, $b=(2\ell^2)^{-1}$, $c=\sqrt{a^2+2ab}$, $A=a+b+c$, $B=b/A$ 이고, $H_k$는 에르미트 다항식<sup>Hermite polynomial</sup>입니다.
